...
export default function Home({ params }: { params: { id: string } }) {
  ...
  if (
    ...
    checkIsMobile(navigator.userAgent || navigator.vendor)
  )
    return (
      <main className="bg-slate-100 dark:bg-slate-900 h-full w-full">
        <div
          className={`w-full h-20 flex justify-between px-5 items-center bg-cyan-700`}
        >
          <p className="text-2xl text-white">Fin-Planning</p>
        </div>
        <div className="w-full h-[calc(100vh-5rem)] overflow-scroll no-scrollbar p-5 text-lg flex align-center justify-center flex-col text-center">
          <div className="w-full flex justify-center p-5">
            <WarningButton />
          </div>
          <p>Sorry, this app is only available on a desktop browser.</p>
        </div>
      </main>
    );
